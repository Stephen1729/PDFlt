    ...
    <div id="action-bar" class="action-bar" style="display:none">
      ...
      <div class="action-buttons">
        ...
        <div class="chain-actions">
          ...
          <button id="reorder-to-merge" class="btn-secondary" title="Unir">➡️ Unir</button>
          <button id="reorder-to-split" class="btn-secondary" title="Separar">➡️ Separar</button>
          <button id="reorder-to-compress" class="btn-secondary" title="Comprimir">➡️ Comprimir</button>
        </div>
      </div>
    </div>
  `
...
async function renderThumbnails(filePath: string, pageCount: number): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      // Calculate scale to fit ~160px width
