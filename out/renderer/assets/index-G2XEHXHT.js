  ...
  container.innerHTML = `
    <div class="home-container">
      <div class="home-header">
        <h1>📄 PDFToolkit</h1>
        <p>Tu herramienta local para manipular PDFs — 100% offline</p>
      </div>

      <div class="features-grid">
        <div class="feature-card" data-view="reorder">
          <span class="icon">🔀</span>
          <h3>Reordenar Páginas</h3>
          <p>Arrastra y suelta para cambiar el orden de las páginas</p>
        </div>

        <div class="feature-card disabled" data-view="merge">
          <span class="icon">📑</span>
          <h3>Unir PDFs</h3>
          <p>Combina varios archivos PDF en uno solo</p>
          <span class="badge-soon">Próximamente</span>
        </div>

        <div class="feature-card disabled" data-view="split">
          <span class="icon">✂️</span>
          <h3>Separar PDF</h3>
          <p>Divide un PDF en varios archivos</p>
          <span class="badge-soon">Próximamente</span>
        </div>

        <div class="feature-card disabled" data-view="compress">
          <span class="icon">📦</span>
          <h3>Comprimir PDF</h3>
          <p>Reduce el tamaño de tus archivos PDF</p>
          <span class="badge-soon">Próximamente</span>
        </div>
      </div>
    </div>
  `;
  container.querySelectorAll(".feature-card:not(.disabled)").forEach((card) => {
...
class CommandManager {
  ...
  add({
    ...
    if (this.#position === -1) {
      ...
      this.#position = 0;
      this.#commands.push(save2);
      return;
    }
    if (overwriteIfSameType && this.#commands[this.#position].type === type) {
      if (keepUndo) {
        save2.undo = this.#commands[this.#position].undo;
      }
      this.#commands[this.#position] = save2;
      return;
    }
  ...
  static setupStorage(html, id, element, storage, intent) {
    ...
    switch (element.name) {
      ...
      case "select":
        if (storedData.value !== null) {
          html.setAttribute("value", storedData.value);
          for (const option2 of element.children) {
...
async function renderThumbnails(filePath, pageCount) {
...
function updatePageOrder() {
  ...
  currentPageOrder = Array.from(cards).map(
    (card) => parseInt(card.dataset.pageIndex, 10)
  );
...
function handleReset() {
  ...
  cards.sort((a, b) => {
    const ai = parseInt(a.dataset.pageIndex, 10);
    const bi = parseInt(b.dataset.pageIndex, 10);
    return ai - bi;
  });
...
async function handleSave() {
  ...
  const isOriginalOrder = currentPageOrder.every((val, idx) => val === idx);
  if (isOriginalOrder) {
    showNotification("El orden no ha cambiado.", "warning");
    return;
  }
  ...
  try {
    const result = await window.electronAPI.reorderPages(currentFilePath, currentPageOrder);
    if (result.success) {
...
function renderComingSoon(container, view) {
  const labels = {
    merge: "Unir PDFs",
    split: "Separar PDF",
